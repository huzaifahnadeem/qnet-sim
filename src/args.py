...
def get_args(): # TODO: bug fix: using the --help flag only prints help for --config_file and not others. This is probably because that is added first and then we parse_known_args and then add others.
    ...
    parser.add_argument('--x_dist_gte', required=False, default=config.x_dist_gte, type=int, help=f'Will force the src-dst pair to have an x-dist of >= this value. Only works for grid topology.')
    parser.add_argument('--y_dist_gte', required=False, default=config.y_dist_gte, type=int, help=f'Will force the src-dst pair to have a  y-dist of >= this value. Only works for grid topology.')
    parser.add_argument('--x_dist_lte', required=False, default=config.x_dist_lte, type=int, help=f'Will force the src-dst pair to have an x-dist of <= this value. Only works for grid topology.')
    parser.add_argument('--y_dist_lte', required=False, default=config.y_dist_lte, type=int, help=f'Will force the src-dst pair to have a  y-dist of <= this value. Only works for grid topology.')
