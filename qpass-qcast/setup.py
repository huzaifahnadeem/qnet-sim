...
def get_args() -> None:
    ...
    parser.add_argument('--network', required=False, default=config.network_toplogy, choices=[nt.value for nt in globals.NET_TOPOLOGY], help=f'The network topology to use. Default set to {config.network_toplogy}.')
    parser.add_argument('--alg', required=False, default=config.algorithm, choices=[a.value for a in globals.ALGS], help=f'Choice between QPASS and QCAST. Default set to {config.algorithm}.')
