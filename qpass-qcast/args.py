...
def get_args(): # TODO: bug fix: using the --help flag only prints help for --config_file and not others. This is probably because that is added first and then we parse_known_args and then add others.
    ...
    parser.add_argument('--qc_noise_param', required=False, default=config.qc_noise_param, type=float, help=f'')
    ...
    parser.add_argument('--qc_p_loss_init', required=False, default=config.qc_p_loss_init, type=float, help=f'The probability of losing the qubit as it enters the channel. Only used with --loss_model=fibre')
    parser.add_argument('--qc_p_loss_length', required=False, default=config.qc_p_loss_length, type=float, help=f'The probability of losing the qubit over the length of the channel. Only used with --loss_model=fibre')
